...
![](normProc.png){width=75%}

![](bernProc.png){width=75%}
...
![](bernProb.png){width=75%}

![](bernLik.png){width=75%}
...
Since we're (mostly) collecting data and trying to guess parameters from it, are we dealing with _probability_ or _likelihood_?
...
The best match (__maximum likelihood__ value) is at $\phi$ = 0.666 (2 heads out of 3 flips)
...
```{r, echo=FALSE, eval = TRUE, fig.width= 3, fig.height = 1.75}
ggplot() + geom_function(fun=llfun) + xlim(0,1) +
  labs(x=expression(phi),y=expression(paste('Likelihood(H,H,T|',phi,')'))) +
...
- Remember: simple linear models take the form:
...
\center Instead of finding $\phi$, R finds the coefficients ($\textcolor{blue}{b_0}$, $\textcolor{blue}{b_1}$ ... $\textcolor{blue}{b_i}$) that create $\phi$
...
# y ~ x, where x is the predictor of y
mod_binomial <- glm(y2 ~ x1 + x2 , data = d1, family = 'binomial') #Fit a binomial GLM
```


```{r, echo=FALSE, eval=TRUE}
mod_binomial <- glm(y2 ~ x1 + x2 , data = d1, family = 'binomial') 
...
- Why is the line not straight? Why are the confidence intervals not symmetrical? 

- Answer: the model is _linear_ on the link scale, but _nonlinear_ on the data scale
...
- Remember that bat data from last week? (Found [here](https://github.com/samuelVJrobinson/ecoStatsLectures/tree/fall2023/03%20Linear%20models) in `batDat.csv`). We used a `lm` last week to fit it, but it actually came from a `glm`
- Fit a `glm` to those data, check how the model fits, and make some partial effects plots
- Recall: this is the "true" causal relationship here (no interactions)

```{r echo=FALSE, fig.fullwidth=TRUE, fig.height=2.5}  
...
batDat <- read.csv('../03 Linear models/batDat.csv')
batsPoisson <- glm(bats~poly(temp,2)+log(light)+bugs,data=batDat,family = 'poisson')

pltThm <- theme_classic()+theme(plot.title=element_blank())

q1 <- ggpredict(batsPoisson,terms='temp') %>% plot(residuals=TRUE)+pltThm
q2 <- ggpredict(batsPoisson,terms='light') %>% plot(residuals=TRUE)+pltThm
q3 <- ggpredict(batsPoisson,terms='bugs') %>% plot(residuals=TRUE)+pltThm

ggarrange(q1,q2,q3,nrow=3,ncol=1)
...
```{r poissonResid, fig.width=6,fig.height=5}
...
text(x=d1$x[resNo],mean(c(exp(d1$yhat[resNo]),y=d1$y1[resNo])),
     labels=paste('Residual = +',round(r1,3)),adj=-0.1,col='red',cex=2)
```
...
plot(predict(m1),residuals(m1,type='response'),#xlab='Predicted value',ylab='Residuals',
     cex=0.75,cex.axis=1e-5,tck=0)
...
qqnorm(residuals(m1,type='response'),col=colVec,pch=pchVec,main=NULL,cex=0.75) 
qqline(residuals(m1,type='response'))
...
- Next week we will deal with _simulation testing_ residuals

```{r poissonResid, fig.width=6,fig.height=4}
```
...
plot(predict(m1),residuals(m1,type='deviance'),cex=0.75,cex.axis=1e-5,tck=0)
abline(h=0,lty='dashed')
...
colVec <- rep('black',nrow(d1)); colVec[resNo] <- 'red'
pchVec <- rep(1,nrow(d1)); pchVec[resNo] <- 19
qqnorm(residuals(m1,type='deviance'),col=colVec,pch=pchVec,main=NULL,cex=0.75)
qqline(residuals(m1,type='deviance'))
...
```{r poissonMod, fig.height=4, fig.width=4}
...
```{r nbModOutput, echo=TRUE, eval=FALSE}
library(MASS) #Required for NB models
m3 <- glm.nb(y2~x,data=d1)
summary(m3) 
```

- Model no longer indicates overdispersion!

::::

:::: {.column width="60%"}

```{r nbModOutput, echo=FALSE, eval=TRUE}
```
...
```{r, nbMod, fig.height=4,fig.width=4}
...
A way to think about this model:
...
```{r ziMod, fig.width=7, fig.height=5}
n <- 200
...
- The modeling approaches for this are somewhat bespoke, so if you need to use this, come talk to me!
...
```{r, ziMod, fig.width=5, fig.height=4}
```
...
"I counted critters for different lengths of time at each site. Does temperature affect counts?"
...
```{r, eval=FALSE, echo=TRUE}
#hours = observation time at each site (must be log-transformed)
...
- Dr. _Paulo Malpern_ (Paul Galpern's evil nemesis) sent 2 people out to check out some bee habitats in Edmonton and Calgary. One of them dutifully counted bees at each site, but the other one was really lazy, and just recorded "bees or no bees" (1 or 0).
- Fit a model to each of the datasets (found [here](https://github.com/samuelVJrobinson/ecoStatsLectures/blob/fall2023/04%20GLMs) in `beeDatGLM.csv`) using a GLM
  - Terms to include: `city` and `floral` (no interaction)
- How do the models look? Compare the coefficients and see if they are different
  - Bonus: make partial regression plots of the terms in each GLM


## Model results

```{r, fig.fullwidth=TRUE, fig.height=5}
dat <- read.csv('./beeDatGLM.csv') #Read in data

#Fit models
modPois <- glm(beeCounts~city+floral,data=dat,family='poisson')
# summary(modPois)

modBin <- glm(beePres~city+floral,data=dat,family='binomial')
# summary(modBin)

#Compare models

actualCoefs <- data.frame(term=names(coef(modBin)),estimate=c(1,-0.2,0.3),
           std.error=0,mod='actual')

list(count=modPois,presence=modBin) %>% 
  lapply(.,tidy) %>% bind_rows(.id = 'mod') %>% 
  select(-statistic,-p.value) %>% 
  bind_rows(actualCoefs) %>% 
  mutate(term=factor(term,labels=c('Intercept','City','Floral'))) %>% 
  ggplot(aes(term,estimate,col=mod))+
  geom_pointrange(aes(ymax=estimate+std.error*2,ymin=estimate-std.error*2),position=position_dodge(width=0.3))+
...
- Some of you have non-normal data... time to update those models!
- For those who don't need to run GLMS, try this:
- I have a dataset of canola plants (found [here](https://github.com/samuelVJrobinson/ecoStatsLectures/blob/fall2023/04%20GLMs) in `canolaPlants.csv`) that I collected during my PhD. I was interested in whether plant size (`VegMass`) and distance from the edge of the field (`Distance`) affected how many flowers got turned into seed pods. I counted pods (`Pods`) on plants, as well as missing pods (`Missing`).
  - Clean up this dataset (there are a couple NAs and unrealistic numbers), fit a GLM that answers my question, and make some plots of your results. Were there any important _interactions_ between the `VegMass` and `Distance`?
