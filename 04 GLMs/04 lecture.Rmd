...
- Fit a model to each of the datasets (found [here](https://github.com/samuelVJrobinson/ecoStatsLectures/blob/fall2023/04%20GLMs) in `batDatGLM.csv`) using a GLM
...
dat <- read.csv('./batDatGLM.csv') #Read in data
...
qqnorm(residuals(m1,type='response'),col=colVec,pch=pchVec,main=NULL) 
...
colVec <- rep('black',nrow(d1)); colVec[resNo] <- 'red'
pchVec <- rep(1,nrow(d1)); pchVec[resNo] <- 19
qqnorm(residuals(m1,type='deviance'),col=colVec,pch=pchVec,main=NULL)
