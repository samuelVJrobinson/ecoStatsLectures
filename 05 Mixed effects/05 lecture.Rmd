...
- Multiplying them looks like this: $$ A \times b = Ab = 1 \times \begin{bmatrix}
1  \\
2 \\
3 
...
fishDat <- read.csv('./fishMass.csv') %>% 
  mutate(across(c(species,sites),factor))
...
- Correlation of fixed effects:
  - Refers to correlation in _estimates_, not in _data_
...
```{r lmerPlot2, fig.height=3.5, fig.fullwidth=TRUE}
dat %>% mutate(pred=predict(mm2),gpred=predict(mm2,re.form=~0)) %>% 
  ggplot(aes(x=x,y=y,col=site))+
  geom_point()+
  geom_line(aes(y=gpred),col='black',linewidth=2) +
...
```{r partPool, fig.fullwidth=TRUE,fig.height=5}
...
bind_rows(a1,a2,.id='mod') %>% mutate(mod=factor(mod,labels=c('fixed','random'))) %>% 
  ggplot(aes(x=x,y=predicted,col=mod))+geom_point(size=3)+
...
ggplot(modPreds,aes(x=x,y=predicted))+geom_ribbon(aes(ymax=conf.high,ymin=conf.low,fill=group),alpha=0.3)+
  ...
  scale_colour_manual(values=colValues)+theme(legend.position = c(0.8,0.25))
```
...
```{r canolaModPlot,fig.width=5,fig.height=6,warning=FALSE,message=FALSE}
...
- If you're testing multiple things at the same time, you need to account for _multiple comparisons_
...
- LMMs and GLMMs are _hard_ to both understand and fit. They are not as forgiving as LMs and GLMs, and will sometimes fail to fit, take a very long time, or give you weird answers without any explanation.
...
- __Avoid selecting models based on $R^2$ or AIC alone. Think about how the system works!__
...
- I have a dataset of honey bee visitation in canola crops (found [here](https://doi.org/10.5061/dryad.vhhmgqnvj)) that I collected during my PhD. I was interested in whether distance from the edge of the field (`Distance`) and number of honey bee hives affected the visitation rate of honey bees.
