...
- If you have a small number of groups, you can just include it in your model: `lm(y ~ x + group)`
  - If you have few samples for each group, this can create problems
...
ggpredict(m1,terms=c('forest','species')) %>% data.frame() %>% 
  ...
  labs(x='Forest cover (%)', y='Mass',col='Species',fill='Species',
       title='Fixed effects model: lm(mass~forest*species)')
...
ggpredict(m2,terms=c('forest','species')) %>% data.frame() %>% 
  ...
  scale_colour_brewer(type = 'qual',aesthetics = c('colour','fill'))+
  labs(x='Forest cover (%)', y='Mass',col='Species',fill='Species',
       title='Mixed effects model: lmer(mass~forest*species+(1|sites))')
...
- You can extract deviance residuals and make your own plots, but I usually use `simulateResiduals` from `DHARMa` (see [here](https://cran.r-project.org/web/packages/glmmTMB/vignettes/model_evaluation.pdf))
...
- This works fairly well across a whole range of models, so I would try to learn it!

```{r dharmaPlot, message=FALSE, warning=FALSE, fig.height=6, fig.fullwidth=TRUE, size='tiny'}
library(DHARMa)
...
```{r glmmPredict, fig.width=7,fig.height=4}
library(ggeffects)
...
- Unfortunately, different numbers of data points change your likelihood, so this can wreck LRTs. _This happens a lot if you have NAs in your predictor columns_, so clean up your data before using it in models.
