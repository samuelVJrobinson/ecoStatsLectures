...
group <- paste('Anne','Bubba','Celia','Doug',sep=', ')
group
...
testData <- read.csv('test_results.csv') #Reads in csv file as data frame.
...
data(iris) #Take a look at the data in 'iris'
head(iris)

plot(iris$Species,iris$Sepal.Length,xlab='Species',ylab='Sepal Length (cm)') #Boxplot of iris data
irismod <- aov(Sepal.Length~Species,data=iris) #Fit an ANOVA model
par(mfrow=c(2,2)) #Changes plotting window to 2x2 grid
plot(irismod) #Plots residuals from model. 
summary(irismod) #Significant difference
TukeyHSD(irismod) #Performs a Tukey test. All 3 are different.

par(mfrow=c(1,1)) #Changes plotting window back to 1x1
plot(iris$Species,iris$Sepal.Length,xlab='Species',ylab='Sepal Length (cm)') #Re-plot
text(x=c(1,2,3)+0.3,y=c(5.5,6.6,7.2),labels=c('A','B','C'),col='red') #Displays grouping letters
...
data(mtcars) #Take a look at the data in 'mtcars'
head(mtcars)

plot(mtcars$hp,mtcars$mpg,xlab='Horsepower',ylab='Miles per gallon',pch=19) #Plot hp vs mpg. Looks like mpg decreases with hp.
carmod <- lm(mpg~log(hp),data=mtcars) #Fit a linear regression, using log(hp)
par(mfrow=c(2,2)) #Changes plotting window to 2x2 grid
plot(carmod) #Plots residuals from model. Looks OK.
summary(carmod) #log(hp) has a strong negative effect on mpg

par(mfrow=c(1,1)) #Change back to a 1x1 window
plot(mtcars$hp,mtcars$mpg,xlab='Horsepower',ylab='Miles per gallon',pch=19) #Re-plot data points

predictions <- data.frame(hp=mtcars$hp,predMPG=predict(carmod,interval='confidence')) #Make new data frame with predictions from fitted model
head(predictions) #FIT is the predicted mean, LWR and UPR are lower and upper confidence intervals. However, look what happens if we try to plot it:
lines(predictions$hp,predictions$predMPG.fit,col='red') #This happens because the data aren't ordered properly, so R is drawing lines between different points on a curve. Let's fix this:

plot(mtcars$hp,mtcars$mpg,xlab='Horsepower',ylab='Miles per gallon',pch=19) #Re-plot data points
predOrder <- order(predictions$hp) #Order of predictions
predOrder #A vector of numbers, giving the order that predictions SHOULD be in
predictions <- predictions[predOrder,] #Reorders predictions and overwrites it
lines(predictions$hp,predictions$predMPG.fit,col='red') #Much better
lines(predictions$hp,predictions$predMPG.lwr,col='red',lty='dashed')
lines(predictions$hp,predictions$predMPG.upr,col='red',lty='dashed')

#Let's add the R^2 value (explained variance) and the equation for the line

#Adding mixtures of symbols and text is complicated. R text uses EXPRESSIONS to represent mathematical terms, but inserting numbers from your data requires you to EVALUATE (get from memory) the actual R^2 value.
rsqr <- round(summary(carmod)$r.squared,2) #R-squared value, rounded to 2 decimal places
r2Label <- bquote(R^2==.(rsqr)) #This is an EXPRESSION, but R EVALUATES anything inside of .()
text(200,29,r2Label)

coefs <- round(coef(carmod),2) #Coefficients from linear regression, rounded to 2 decimal places
eqnLabel <- bquote(MPG==e^{.(coefs[1])+(.(coefs[2]))%*%HP}) #Same thing as above, but uses coefficients from COEFS
text(243,30,eqnLabel)
...
#Normal rabbits (rmax=1)
rabbits <- data.frame(Time=1:20,P=NA)
rabbits$P[1] <- 2
rmax <- 1
K <- 100

for(i in 2:nrow(rabbits)){
  r <- rmax*(1-(rabbits$P[i-1]/K)) #Rabbits born this year
  rabbits$P[i] <-round(rabbits$P[i-1]+(rabbits$P[i-1]*r)) #Discrete form
}

#Plot rabbit population over 20 years
plot(rabbits$Time,rabbits$P,type='l',ylab='Number of rabbits',xlab='Time',ylim=c(0,125))
points(rabbits$Time,rabbits$P,pch=19) 

#Very reproductive rabbits (rmax=2.5)
rabbits <- data.frame(Time=1:20,P=NA)
rabbits$P[1] <- 2
rmax <- 2.5
K <- 100
...
#Plot rabbit population over 20 years
lines(rabbits$Time,rabbits$P,type='l',col='blue',ylab='Number of rabbits',xlab='Time')
points(rabbits$Time,rabbits$P,pch=19,col='blue') 
abline(h=100,lty='dashed',col='red') #Show K value
legend('bottomright',c('rmax=1','rmax=2.5'),fill=c('black','blue'))
