...
# lon <- model.matrix(~g-1) %*% lon
# time <- model.matrix(~g-1) %*% time
dat3 <- data.frame(count=y,temperature=x+10,site=g,days=factor(time)) #Assemble into data frame

write.csv(dat3,'./timeDat.csv',row.names = FALSE)
...
\textcolor{red}{\Sigma} = & \textcolor{red}{\sigma}^2 \times e^{-\textcolor{ForestGreen}{\rho}^2 Dist^2}\\
\end{split}
\end{equation*}

- Instead of finding a single $\textcolor{red}{\sigma}$ value, R now looks for $\textcolor{red}{\sigma}$ (maximum covariance) and $\textcolor{ForestGreen}{\rho}$ (decay with distance)
...
```{r, fig.height=3}
ggplot(dat3,aes(x=as.numeric(days),y=count))+geom_point()+
  labs(x='Days',y='Count')
```
...
```{r}
nd <- with(dat3,
   expand.grid(temperature=seq(0,20,1),
                 site=sort(unique(site)),dayF=dayF[1]))

data.frame(mod=c('1. Actual','2. Standard','3. Autocorr'),
           coef=c(1.5,fixef(m2)[[1]][2],fixef(m2_ac)[[1]][2]),
...
\begin{equation*}
 ...
 \end{pmatrix} \\
 \sigma_{i,j} = & \textcolor{ForestGreen}{\alpha}^2\text{exp}(-\frac{1}{2\textcolor{ForestGreen}{\rho}^2}D^2_{i,j}) 
 \end{split}
\end{equation*}
...
\begin{equation*}
  ...
  y \sim & Normal (\textcolor{orange}{\hat{y}}, \sigma) \\
...
- For my PhD work, I studied bees in canola fields. One of the species I studied was _Megachile rotundata_, the alfalfa leafcutting bee. I counted bees visiting flowers at distances away from their shelters ("hives"), and some of the data is found [here](https://github.com/samuelVJrobinson/ecoStatsLectures/tree/fall2023/08%20Spatiotemporal%20models) (`seedFieldDat.csv`).
