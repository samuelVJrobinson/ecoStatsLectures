...
- Zillions of different extensions available [here](https://exts.ggplot2.tidyverse.org/gallery/)
...
```{r, eval=FALSE, echo = TRUE}
# Top line of code says: 
# - map gear (number of gears as a factor) to x-axis
ggplot(data = mtcars, aes(x = factor(gear)))+ 
  geom_bar() 
# Display number of data points for each factor level
# Automatically uses stat='count' to group all factors
```
...
ggplot(mtcars,aes(x=factor(am),y=mpg))+ 
  ...
  geom_point(stat='summary',fun=mean,col='orange',size=3) #Mean only
```
...
ggplot(arrange(mtcars,am,disp),aes(x=factor(am),y=mpg))+ 
  ...
  geom_pointrange(stat='summary',fun.data=mean_se,
                  fun.args = list(mult = 2),col='orange') #Mean + 2 SE
```
...
p4 <- ggplot(starwars,aes(x=height))+geom_histogram()+facet_wrap(~sex)+ theme_minimal()
