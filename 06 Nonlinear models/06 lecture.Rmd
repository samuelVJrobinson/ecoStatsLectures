...
The logistic growth model $n_t = n_{t-1}(1+ r(1-\frac{n_{t-1}}{k}))$ has the definite solution:
...
bDat %>% mutate(predA=lgFun(t=year,K=opt3$par[1],r=opt3$par[2],n0=1e-4),
                predB=lgFun(t=year,K=opt4$par[1],r=opt4$par[2],n0=1e-4)) %>% 
  #Divides
...
\begin{equation*}
  ...
  \text{Yield} \sim & Normal (\textcolor{orange}{\hat{y}}, \sigma) \\
...
- If your _x_ values are circular (e.g. days of the year), you can use _circular_ cubic splines (`cc`) 
...
## To do: final nonlinear modeling challeng
