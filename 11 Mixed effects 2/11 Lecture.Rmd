...
subtitle: "Even more bells and whistles"
...
y2 <- rnbinom(n,exp(yhat),sigmaR) #Data (NB)
...
```{r}
par(mfrow=c(2,3))
plot(fitted(mm1),resid(mm1,type='working'),xlab='Fitted values',ylab='Working residuals'); abline(h=0)
qqnorm(resid(mm1,type='working'),main='Residuals');qqline(resid(mm1,type='working'))
qqnorm(ranef(mm1)$site[,1],main='Random Intercepts');qqline(ranef(mm1)$site[,1])
qqnorm(ranef(mm1)$site[,2],main='Random Slopes');qqline(ranef(mm1)$site[,2])
plot(ranef(mm1)$site,xlab='Random Intercept',ylab='Random Slope',main='Intercept-Slope Correlation')
# par(mfrow=c(1,1))
```
...
- `glmmTMB` from `library(glmmTMB)` works for those above, _plus_ a suite of others
