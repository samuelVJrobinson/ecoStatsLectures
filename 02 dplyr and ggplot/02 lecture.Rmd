...
Here's some data in both __wide__ and __long__ format:
...
__wide__ format: data for each "unit" listed in multiple columns
...
__long__ format: data listed in single column, plus and ID column
...
Change long dataframe to wide (reverse of _pivot_longer_)
...
- Hint: get summary stats (mean, SD), then `pivot_longer`, and finally `pivot_wider`
...
```{r, histPlot, eval=FALSE, echo=TRUE, warning=FALSE}
...
```{r, histPlot, eval=TRUE,echo=FALSE, warning=FALSE, message=FALSE}
```
...
You all brought some of your own data... didn't you??\footnote{If not, type `data()` in console to see some built-in R datasets}

\pause

Make a simple plot of some of your data! Don't worry if it's messy or weird

## Colours in plots

- Colours can be _mapped_ (via `aes`) or _set_ (outside of `aes`)
- _mapping_ associates a variable with a colour scheme, _setting_ fixes the colour to a preset value
...
- `scale_colour_brewer` is generally pretty good; see examples [here](https://ggplot2.tidyverse.org/reference/scale_brewer.html) 
- ~10% of (European) males are red-green colourblind; see [here](https://colorbrewer2.org/) for some suggested schemes

\pause

```{r, themePlot, echo=TRUE, eval = FALSE}
ggplot(data=mtcars,aes(x=disp,y=mpg))+
  geom_point(aes(col=factor(gear)))+
  scale_colour_manual(
...
```{r, themePlot, echo=FALSE, eval = TRUE, fig.width=5, fig.height=5}
```
...
- Sizes of things can also be _mapped_ (via `aes`) or _set_ (outside of `aes`), similar to colours. You can alter mapped sizes using `scale_size`
...
```{r, uglyPlot, echo = TRUE, eval=FALSE}
ggplot(data=mtcars,aes(x=disp,y=mpg))+
  # Maps gear to colour
  geom_point(aes(col=factor(gear))) +
  #Changes plot theme
...
- This plot is hideous, but it gives you the idea; use `?theme` to see all options

::::

:::: column

```{r, uglyPlot, echo = FALSE, eval=TRUE, fig.width=4,fig.height=4}
```
::::
...
```{r, customTheme, echo = TRUE, eval=FALSE}
...
- You can break up the plot into smaller `facets` that are mapped to a given variable

```{r, echo = TRUE, eval = TRUE, fig.width=8,fig.height=3.5}
...
p3 <- ggplot(d1, aes(x="", y=y, fill=x)) + geom_bar(width = 1, stat = "identity")+
  coord_polar("y") + theme_void() + theme(axis.text.x=element_text(size=0))
...
```{r eval=TRUE, echo=FALSE, message=FALSE, warning=FALSE, fig.height=4, fig.width=8}
...
p3 <- ggplot(trees,aes(x=Girth,y=Height,col=Volume,size=Volume))+
  ...
  guides(colour = "none") #Turns off colour legend
...
```{r, filtPlot, echo=T,eval=F,message=F,warning=F}
...
```{r, filtPlot, echo=F,eval=T,message=F,warning=F,fig.height = 4.5, fig.width = 6}
```
...
```{r, co2plot, echo=T,eval=F,message=F,warning=F}
...
```{r, co2plot, eval=TRUE,fig.height = 6, fig.width = 5}
```
...
## To do this week: a study in contrasts

Using that dataset that you acquired: 

1. Get some summary statistics for your data (e.g. group means, SDs) using `dplyr`
2. Make a "presentation quality" plot of some of your data
3. Make the ugliest, most unreadable plot in human history. A prize will be awarded for the worst!
